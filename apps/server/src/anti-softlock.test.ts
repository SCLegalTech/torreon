...
describe("Anti-softlock: la retirada táctica", () => {
  ...
  beforeEach(async () => {
    ...
    await store.init();
    service = new QuestService(store, undefined, directory, "torreon-softlock-test", fixedClock(RELOJ_DEL_REINO));
  });
...
describe("Objetos: el frente se nombra", () => {
  ...
  beforeEach(async () => {
    ...
    store = new JsonRealmStore(join(directory, "state.json"), fixedClock(RELOJ_DEL_REINO));
    await store.init();
    service = new QuestService(store, undefined, directory, "torreon-target-test", fixedClock(RELOJ_DEL_REINO));
  });
...
describe("Frentes abiertos: la ruta normal a cualquier Battle", () => {
  ...
  beforeEach(async () => {
    ...
    store = new JsonRealmStore(join(directory, "state.json"), fixedClock(RELOJ_DEL_REINO));
    await store.init();
    service = new QuestService(store, undefined, directory, "torreon-fronts-test", fixedClock(RELOJ_DEL_REINO));
  });
